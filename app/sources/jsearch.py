...
DEFAULT_QUERIES = (
    ...
    "Generative AI intern in India",
    "Machine Learning Engineer intern in Bengaluru",
)


class JSearchJobSource(JobSource):
    """OpenWebNinja JSearch adapter with safe, deterministic normalization."""

    def __init__(
        self,
        name: str = "JSearch API (OpenWebNinja)",
    ...
    def _monthly_amount(min_salary: Any, max_salary: Any, period: Any, currency: str) -> int | None:
        """Return a monthly amount only when the source supplies a usable numeric salary."""
        values = [value for value in (min_salary, max_salary) if isinstance(value, (int, float))]
        if not values:
            return None
        # The canonical Job field is INR. Do not invent FX conversions for non-INR data.
    ...
    def _parse_item(self, item: dict[str, Any]) -> Job | None:
        ...
        # A listing without the minimum identity fields is not useful to the pipeline.
    ...
    def fetch_jobs(self) -> list[Job]:
        ...
        for query in self.queries:
            ...
            if not isinstance(items, list):
                logger.warning("Source %s returned an invalid data payload for query %r", self.name, query)
                continue
            ...
            for item in items:
                ...
                try:
                    ...
                    logger.warning("Source %s returned an invalid job: %s", self.name, exc)
                    continue
                if job is None:
                    continue
                dedup_key = job.external_id or job.application_url
