...
from app.services.resume import build_profile_from_resume, extract_resume_text, resume_status
...
def get_resume_status() -> dict: return resume_status()


@router.post("/resume")
async def upload_resume(file: UploadFile = File(...)) -> dict:
    if not file.filename: raise HTTPException(status_code=400, detail="Resume filename is missing.")
    raw = await file.read()
    if len(raw) > 8 * 1024 * 1024: raise HTTPException(status_code=413, detail="Resume must be 8 MB or smaller.")
    try:
        text_content = extract_resume_text(file.filename, raw)
        profile = build_profile_from_resume(text_content, file.filename)
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {"message": "Resume loaded. Future discovery runs will use this profile.", "resume": resume_status(), "profile": profile}


@router.get("/profile")
def profile() -> dict: return load_profile()


@router.post("/score", response_model=ScoredJob)
def score(job: Job) -> ScoredJob:
    normalized = normalize_job(job); return ScoredJob(job=normalized, score=score_job(normalized))


@router.get("/discovery/urls")
def discovery_urls() -> list[dict[str, str]]:
    profile_data = load_profile(); return build_search_urls(profile_data.get("target_roles", []), profile_data.get("locations", []))
...
def run_discovery(db: Session = Depends(get_db)) -> DiscoveryRunResult:
    try: return run_discovery_pipeline(db)
    except Exception as exc: raise HTTPException(status_code=500, detail=f"Discovery pipeline failed: {exc}") from exc


@router.delete("/jobs")
def clear_jobs(db: Session = Depends(get_db)) -> dict[str, int]:
    deleted = db.query(JobModel).delete(synchronize_session=False); db.commit(); return {"deleted": deleted}
...
def get_jobs(min_score: float = Query(0.0, ge=0.0, le=100.0), location: Optional[str] = Query(None, max_length=255), remote_only: bool = False, international_only: bool = False, limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db)):
    ...
    if remote_only: query = query.filter(JobModel.work_mode.ilike("%remote%"))
    if international_only: query = query.filter(JobModel.location_category == "international_remote")
    if location: query = query.filter(JobModel.location.ilike(f"%{location}%"))
    rows = query.order_by(JobModel.score_total.desc(), JobModel.created_at.desc()).limit(limit).all()
    return [{"id": j.id, "external_id": j.external_id, "title": j.title, "company": j.company, "description": j.description, "location": j.location, "country": j.country, "work_mode": j.work_mode, "remote_category": j.remote_category, "location_category": j.location_category, "stipend_monthly_inr": j.stipend_monthly_inr, "salary_text": j.salary_text, "currency": j.currency, "source": j.source, "application_url": j.application_url, "skills": _json_list(j.skills_json), "perks": _json_list(j.perks_json), "posted_date": j.posted_date, "deadline": j.deadline, "score_total": j.score_total, "score_breakdown": _json_dict(j.score_breakdown_json)} for j in rows]


@router.post("/jobs", status_code=201)
def create_job(job: Job, db: Session = Depends(get_db)):
    normalized = normalize_job(job); normalized.dedup_hash = compute_dedup_hash(normalized)
    existing = db.query(JobModel).filter(JobModel.dedup_hash == normalized.dedup_hash).first()
    if existing: return {"id": existing.id, "title": existing.title, "score_total": existing.score_total, "created": False}
    ...
    db.add(db_job)
    try: db.commit()
    except IntegrityError as exc: db.rollback(); raise HTTPException(status_code=409, detail="Job already exists or violates a database constraint.") from exc
    db.refresh(db_job); return {"id": db_job.id, "title": db_job.title, "score_total": db_job.score_total, "created": True}


@router.get("/jobs/{id}")
def get_job_detail(id: str, db: Session = Depends(get_db)):
    j = db.query(JobModel).filter(JobModel.id == id).first()
    if not j: raise HTTPException(status_code=404, detail="Job not found")
    return {"id": j.id, "external_id": j.external_id, "title": j.title, "company": j.company, "description": j.description, "location": j.location, "country": j.country, "work_mode": j.work_mode, "remote_category": j.remote_category, "location_category": j.location_category, "stipend_monthly_inr": j.stipend_monthly_inr, "salary_text": j.salary_text, "currency": j.currency, "source": j.source, "application_url": j.application_url, "skills": _json_list(j.skills_json), "perks": _json_list(j.perks_json), "posted_date": j.posted_date, "deadline": j.deadline, "score_total": j.score_total, "score_breakdown": _json_dict(j.score_breakdown_json), "llm_analysis": _json_dict(j.llm_analysis_json)}


@router.post("/analyze/job", response_model=JobAnalysisResult)
def analyze_job(job: Job): return get_llm_provider().analyze_job_description(normalize_job(job))


@router.post("/applications", response_model=ApplicationResponse)
def create_app(data: ApplicationCreate, db: Session = Depends(get_db)):
    if not db.query(JobModel).filter(JobModel.id == data.job_id).first(): raise HTTPException(status_code=404, detail=f"Job with ID {data.job_id} not found.")
    try: return create_application(db, data)
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.get("/applications", response_model=list[ApplicationResponse])
def get_apps(status: Optional[str] = None, db: Session = Depends(get_db)):
    try: return list_applications(db, status=status)
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def update_app_route(id: str, data: ApplicationUpdate, db: Session = Depends(get_db)):
    try: result = update_application(db, id, data)
    except ValueError as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
    if not result: raise HTTPException(status_code=404, detail="Application not found")
    return result
...
def generate_questions(req: QuestionGenerationRequest): return generate_application_answers(req)


@router.post("/browser/prepare", response_model=BrowserPrepareResponse)
def prepare_browser(req: BrowserPrepareRequest): return prepare_application_workflow(req)


@router.post("/browser/submit")
def submit_browser(req: BrowserSubmitRequest): return submit_application_workflow(req)
