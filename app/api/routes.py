...
from app.services.resume import build_profile_from_resume, extract_resume_text, get_profile_dict, resume_status, save_profile_to_db
...
def sources_health() -> list[dict[str, Any]]:
    ...
    result = []
    for s in sources:
        status = getattr(s, "last_status", "Ready")
        err = getattr(s, "last_error", "")
        if hasattr(s, "api_key") and not getattr(s, "api_key"):
...
async def upload_resume(file: UploadFile = File(...)) -> dict:
    ...
    try:
        ...
        profile_data = build_profile_from_resume(text_content, file.filename)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {
        "message": "Resume uploaded and candidate profile updated.",
...
def update_profile(data: dict) -> dict:
    ...
    return {"message": "Candidate profile updated successfully.", "profile": get_profile_dict()}
...
def clear_jobs(db: Session = Depends(get_db)) -> dict[str, int]:
    deleted = db.query(JobModel).delete(synchronize_session=False)
    db.commit()
...
def get_job_detail(id: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "score_total": j.score_total,
        "score_breakdown": _json_dict(j.score_breakdown_json),
        "llm_analysis": _json_dict(j.llm_analysis_json),
    }
